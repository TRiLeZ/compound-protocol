...
contract ComptrollerG7 is ComptrollerV6Storage, ComptrollerInterface, ComptrollerErrorReporter, ExponentialNoError {
    ...
    /// @notice Emitted when supply COMP rate is changed
    event NewCompSupplyRate(uint oldCompRate, uint newCompRate);

    /// @notice Emitted when borrow COMP rate is changed
    event NewCompBorrowRate(uint oldCompRate, uint newCompRate);
    ...
    function refreshCompSpeedsInternal() internal {
        ...
        for (uint i = 0; i < allMarkets_.length; i++) {
            ...
            uint _newSupplySpeed = totalUtility.mantissa > 0 && _totalSupplyWeight > 0 ?
                mul_(compSupplyRate, mul_(div_(utilities[i], totalUtility), div_(compSupplyWeights[address(cToken)], _totalSupplyWeight))) : 0;
            ...
            uint _newBorrowSpeed = totalUtility.mantissa > 0 && _totalBorrowWeight > 0 ?
                mul_(compBorrowRate, mul_(div_(utilities[i], totalUtility), div_(compBorrowWeights[address(cToken)], _totalBorrowWeight))) : 0;
